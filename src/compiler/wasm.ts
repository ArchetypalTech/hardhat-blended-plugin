...
export function compileRustToWasm(projectPath: string, outputDir: string, pkgName: string): string {
  ...
  try {
    execSync(`cargo build --release --target=wasm32-unknown-unknown --no-default-features`, {
      cwd: projectPath,
